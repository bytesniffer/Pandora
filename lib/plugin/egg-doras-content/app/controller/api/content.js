...
const ContentController = {
  ...
  async list(ctx, app) {
    try {
      ...
      if (!_.isEmpty(userInfo) && userInfo.id === userId) {
        ...
        // 不区分状态
        if (listState === 'all' || listState === '-1') {
          delete queryObj.state;
        } else {
          if (listState === '0' || listState === '1' || listState === '2' || listState === '3' || listState === '4'
            || listState === '5') {
  ...
  // 更新状态
  async transferContentState(ctx) {
      try {
        const reqJson = ctx.request.body;
        console.log('Transfer state:' + JSON.stringify(reqJson)+',id:'+(typeof reqJson.id === 'number'));
        const targetContent = await ctx.service.content.item({
          query: {
            author_id: ctx.session.user.id,
            id: reqJson.id,
        ...
        var query = {}
        if(!_.isEmpty(reqJson.expectedState)){
           query = _.assign({},query,{state:reqJson.expectedState})
        }
        await ctx.service.content.updateByCondition(parseInt(reqJson.id), updateObj, query);
        ctx.helper.renderSuccess(ctx, {
          data: {},
        });
      } catch (err) {
        console.log('transfer state err:' +err)
        ctx.helper.renderFail(ctx, {
          message: err,
        });
      }
