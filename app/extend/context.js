...
module.exports = {
  async getSiteInfo() {
    ...
    const { title, des, keywords } = ctxParams;
  ...
  async getPageData() {
    ...
    // 获取用户信息
    if (ctx.session.logined && !_.isEmpty(ctx.session.user)) {
    ...
    if (!_.isEmpty(pageData.site)) {
      ...
      if (ctx.pageType === 'index' || ctx.pageType === 'all') {
        // 首页
        ...
        pageData.posts = docs;
        pageData.pageInfo = pageInfo;
      } else if (ctx.pageType === 'tags') {
        // 标签所属
        const docs = await ctx.helper.reqJsonData('contentTag/getList', {
          isPaging: '0',
        });
        pageData.tags = docs;
