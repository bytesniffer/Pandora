...
const ContentController = {
  ...
  async create(ctx) {
    try {
      ...
      if (fields.content_type === '1' || fields.content_type === '3' ) {
  ...
  async update(ctx) {
    ...
    async transferContentState(ctx) {
        try {
          ...
          console.log('Transfer state:' + JSON.stringify(reqJson) + ',query:' + JSON.stringify(query)+',id:'+(typeof reqJson.id === 'number') )
          await ctx.service.content.updateByCondition(reqJson.id, updateObj, query);
          ctx.helper.renderSuccess(ctx, {
            data: {},
          });
        } catch (err) {
