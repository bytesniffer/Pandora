...
export default {
  ...
  computed: {
    ...
    editorState() {
      let defaultEditor = '';
      if (this.appConfig) {
        defaultEditor = this.appConfig.editorType;
      }
      if (this.formState.edit) {
        return this.formState.formData.content_type || '1';
      } else {
        ...
        if (result.status === 200) {
          if (result.data) {
            ...
            if (contentObj.categories) {
              contentObj.categories = _.sortBy(contentObj.categories, item => {
            ...
            if (contentObj.content_type === '2' && contentObj.comments) {
              this.mdEditorHtml = contentObj.comments;
            }
