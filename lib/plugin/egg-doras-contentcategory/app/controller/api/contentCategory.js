...
const ContentCategoryController = {
  ...
  async treelist(ctx) {
    ...
    for (var index in renderCates) {
              renderCates[index].children = _.sortBy(renderCates[index].children, function (item) {
                                                      return item.sortId;
                                                  });
    }
    ctx.helper.renderSuccess(ctx, {
            ...
            if (typeId || !_.isEmpty(contentObj)) {
                ...
                if (parentObj.length > 0) {
                    ...
                    cates = _.filter(fullNav, (doc) => {
                        return (doc.sortPath).indexOf(parentId) > 0 && doc.parentId != '0' && doc.enable
                    });
                    parents = _.filter(fullNav, (doc) => {
                        return (doc.sortPath).indexOf(parentId) > 0 && doc.parentId === '0' && doc.enable
                    });
                }
            }
